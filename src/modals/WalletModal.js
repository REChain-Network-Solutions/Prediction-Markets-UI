...
export const WalletModal = ({ children = "WALLET", type = "default", styles = {} }) => {
  ...
  const handleWalletAddress = (ev) => {
    if (oREChain.utils.isValidAddress(ev.target.value)) {
      ReactGA.event({
        category: "user-engagement",
        action: "save-wallet"
      });
    ...
    setWalletAddress({
      valid: oREChain.utils.isValidAddress(ev.target.value),
    ...
    >
      ...
      <Form size="large">
        <Form.Item validateStatus={walletAddress.value === "" ? "" : (walletAddress.valid ? "success" : "error")} extra={<small style={{ fontSize: 12 }}>
          <Trans i18nKey="modals.wallet.install_oREChain"><a href="https://oREChain.org/#download" target="_blank" rel="noopener">Install OREChain wallet</a> if you don't have one yet, and copy/paste your address here.</Trans>
        </small>}>
