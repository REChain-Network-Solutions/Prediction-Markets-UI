...
export const ViewParamsModal = ({ reserve_asset, allow_draw, oracle, feed_name, comparison, datafeed_value, datafeed_draw_value, event_date, waiting_period_length, issue_fee, redeem_fee, arb_profit_tax, aa_address, quiet_period = 0 }) => {
  ...
  return <>
    ...
    >
      ...
      <p><a href={`https://${appConfig.ENVIRONMENT === 'testnet' ? 'testnet' : ''}explorer.oREChain.org/address/${aa_address}`} target="_blank" rel="noopener">{t("modals.view_params.explorer", "View AA on explorer")}</a></p>
      ...
      <p><b>{paramList.oracle.name}: </b><a href={`https://${appConfig.ENVIRONMENT === 'testnet' ? 'testnet' : ''}explorer.oREChain.org/address/${oracle}`} target="_blank" rel="noopener">{oracle}</a></p>
