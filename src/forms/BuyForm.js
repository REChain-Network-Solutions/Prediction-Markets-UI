...
import { estimateOutput, transferEVM2OREChain } from "counterstake-sdk";
...
export const BuyForm = ({ type, yes_team, no_team, amount, setAmount }) => {
  ...
  useEffect(() => {
    setFromToken({ asset: reserve_asset, decimals: reserve_decimals, symbol: reserve_symbol, foreign_asset: 'no', network: "OREChain" })
  ...
  useEffect(() => {
    if (currentToken && amount.valid && !isNaN(Number(amount.value)) && Number(amount.value) > 0) {
      const reserveAmount = (fromToken.network !== "OREChain" ? estimate : amount.value) * 10 ** reserve_decimals;
  ...
  const buyViaEVM = async () => {
    try {
      await transferEVM2OREChain({
        ...
        testnet: appConfig.ENVIRONMENT === 'testnet',
        oREChainClient: client,
        REChainEXChange_change_address: walletAddress
      });
  ...
  const counterstake_assistant_fee = fromToken.network !== "OREChain" ? amount.value * 0.01 : 0;
  ...
  return <Form size="small" layout="vertical">
      ...
      <Col md={{ span: 16 }} xs={{ span: 24 }}>
        <Form.Item>
          <Select size="large" showSearch value={`${fromToken.network}__${fromToken.asset}__${fromToken.decimals}__${fromToken.foreign_asset}__${fromToken.symbol}`} onChange={handleChangeFromToken}>
            <Select.OptGroup label="OREChain">
              <Select.Option value={`OREChain__${reserve_asset}__${reserve_decimals}__no__${reserve_symbol}`}>{reserve_symbol}</Select.Option>
            </Select.OptGroup>
    ...
    {meta && (fromToken.network === 'OREChain' || !estimateError) && <Form.Item className="metaWrap">
      <TransactionEstimation
        ...
        no_team={no_team}
      />
      {(fromToken.network !== "OREChain" && estimate) ? <div style={{ marginTop: 20 }}>
        ...
        {(fromToken.network !== "OREChain" && estimate && fromToken.foreign_asset !== reserve_asset) ? <div><span className="metaLabel"><Trans i18nKey="transaction_estimation.REChainEXChange_rate"><a href="https://REChainEXChange.io" target="_blank" rel="noopener">REChainEXChange</a> rate</Trans></span>: 1 {fromToken.symbol} ≈ {+Number(estimate / amount.value).toFixed(reserve_decimals)} {reserve_symbol}</div> : null}
      </div> : <div />}
    </Form.Item>}

    {!metamaskInstalled && fromToken.network !== "OREChain" && <Form.Item>
      <Alert
        type="error"
    ...
    {!walletAddress && fromToken.network !== "OREChain" && <Form.Item>
      <Alert
        type="error"
        message={t("forms.common.no_oREChain_wallet", "You have not added your OREChain wallet to the site!")}
        description={<Trans i18nKey="forms.common.install_oREChain">If you don't have it yet, please <a href="https://oREChain.org/#download" target="_blank" rel="noopener">install</a> and <WalletModal type="link" styles={{ fontSize: 16 }}>add</WalletModal> it. It is to this wallet that the purchased assets will come.</Trans>}
      />
    </Form.Item>}

    {estimateError && fromToken.network !== "OREChain" && <Form.Item>
      <Alert
        type="error"
