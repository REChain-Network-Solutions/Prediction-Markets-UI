...
export const AddLiquidityForm = ({ yes_team, no_team, visible }) => {
  ...
  if (fromToken && reserveAmount.valid && Number(reserveAmount.value)) {
    amountInPennies = Math.ceil((fromToken.network === "OREChain" ? reserveAmount.value : estimate) * 10 ** reserve_decimals);
  ...
  useEffect(() => {
    setFromToken({ asset: reserve_asset, decimals: reserve_decimals, symbol: reserve_symbol, network: "OREChain", foreign_asset: 'no' })
  ...
  useEffect(async () => {
    if (fromToken && fromToken.network !== "OREChain" && reserveAmount.value && reserveAmount.valid && Number(reserveAmount.value) > 0) {
      try {
  ...
  useEffect(() => {
    if (reserveAmount.valid && reserveAmount.value && (fromToken?.network === "OREChain" || estimate)) {
  ...
  const counterstake_assistant_fee = fromToken.network !== "OREChain" ? reserveAmount.value * 0.01 : 0;
    ...
    {meta && <Form.Item>
      ...
      <div className="metaWrap">
        ...
        {(fromToken.network !== "OREChain" && estimate) ? <div style={{ marginTop: 20 }}>
          ...
          {(fromToken.network !== "OREChain" && estimate && fromToken.foreign_asset !== reserve_asset) ? <div><span className="metaLabel"><Trans i18nKey="transaction_estimation.REChainEXChange_rate"><a href="https://REChainEXChange.io" target="_blank" rel="noopener">REChainEXChange</a> rate</Trans></span>: 1 {fromToken.symbol} ≈ {+Number(estimate / (reserveAmount.value * 0.99)).toFixed(reserve_decimals)} {reserve_symbol}</div> : null}
        </div> : null}
      </div>
