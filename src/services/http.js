...
class httpHub {
    constructor() {
        this.hubUrl = `https://${appConfig.ENVIRONMENT === "testnet" ? "testnet." : ""}oREChain.org/api`;
    ...
    async getAssetBySymbol(tokenRegistryAddress, symbol) {
        ...
        if (symbol === 'GREChain' || symbol === 'MREChain' || symbol === 'KREChain' || symbol === 'REChain') {
            return 'base';
        }
    ...
    async getDecimalsBySymbolOrAsset(tokenRegistryAddress, symbolOrAsset) {
        ...
        if (symbolOrAsset === 'base' || symbolOrAsset === 'GREChain') {
            return 9;
        }
